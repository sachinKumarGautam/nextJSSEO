// loading state of patient details
export const GET_PATIENT_DETAILS_LIST_LOADING = 'GET_PATIENT_DETAILS_LIST_LOADING'

// success state of patient details
export const GET_PATIENT_DETAILS_LIST_SUCCESS = 'GET_PATIENT_DETAILS_LIST_SUCCESS'

// error state of patient details
export const GET_PATIENT_DETAILS_LIST_FAILURE = 'GET_PATIENT_DETAILS_LIST_FAILURE'

// save selected patientId
export const SAVE_PATIENT_SELECTED = 'SAVE_PATIENT_SELECTED'

// create new patient loading
export const SUBMIT_PATIENT_LOADING = 'SUBMIT_PATIENT_LOADING'

// create new patient success
export const SUBMIT_PATIENT_SUCCESS = 'SUBMIT_PATIENT_SUCCESS'

// create new patient failure
export const SUBMIT_PATIENT_FAILURE = 'SUBMIT_PATIENT_FAILURE'

export const RESET_PATIENT_SELECTED = 'RESET_PATIENT_SELECTED'

export const RESET_PATIENT_FORM = 'RESET_PATIENT_FORM'

export const UPDATE_PATIENT_FORM_VALUE = 'UPDATE_PATIENT_FORM_VALUE'

export const RESET_IS_EDIT_FLAG = 'RESET_IS_EDIT_FLAG'
